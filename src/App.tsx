...
import { exportFinanceStateJson, parseFinanceStateJson } from "./lib/storage";
...
function getFirebaseStateLabel(firebaseState: ReturnType<typeof useFinanceData>["firebaseState"]) {
  if (firebaseState === "loading") return "Cargando Firebase...";
  if (firebaseState === "saving") return "Guardando en Firebase...";
  if (firebaseState === "connected") return "Firebase sincronizado";
  if (firebaseState === "error") return "Error de sincronizacion";
  if (firebaseState === "idle") return "Sesion lista";
  return "Modo local";
}

export default function App() {
  const { sessionState, userEmail, userId, authError, isSubmitting, login, register, logout } = useFirebaseSession();
  const { financeState, dataSource, firebaseState, persistState } = useFinanceData(
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      setFeedback("Sincronizacion activa");
    }, 2400);
  ...
  const currentMonthLabel = formatMonthLabel(todayKey());
  ...
  const handleExportBackup = () => {
    const payload = exportFinanceStateJson(financeState);
    const blob = new Blob([payload], { type: "application/json" });
    const fileUrl = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    const timestamp = todayKey();

    anchor.href = fileUrl;
    anchor.download = `finances-backup-${timestamp}.json`;
  ...
  const handleImportFile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const nextState = parseFinanceStateJson(rawValue);
      await persistState(nextState, financeState);
      resetForm();
      setFeedback("Backup importado en Firebase");
    } catch (error) {
      console.error("No se pudo importar el backup", error);
      setFeedback("No se pudo importar el backup");
    } finally {
      event.target.value = "";
    }
  };
    ...
    <div className="page-shell">
      ...
      <header className="topbar">
        ...
        <div className="topbar__actions">
          ...
          />
          <span className="status-pill status-pill--neutral">{feedback}</span>
          <span className={syncPillClass}>{dataSourceLabel}</span>
          {userEmail ? <span className="status-pill status-pill--neutral">{userEmail}</span> : null}
          <span className="status-pill status-pill--today">{currentMonthLabel}</span>
          <button type="button" className="outline-button" onClick={() => importInputRef.current?.click()}>
            Importar backup
          </button>
          <button type="button" className="outline-button" onClick={handleExportBackup}>
            Exportar backup
          </button>
      ...
      <section className="category-grid">
        ...
        <article className="surface-card category-card category-card--variable">
          <div className="section-heading">
            ...
            <span className="status-pill status-pill--section status-pill--section-variable">{metrics.variableExpenses.activeCount} activos</span>
          </div>
          <div className="category-card__stats">
            <div>
              <span>Pagado este mes</span>
          ...
          <p className="category-card__note">Pensado para agua, luz o cualquier gasto cuyo monto cambia. Editas el valor del ciclo y luego pagas.</p>
        </article>
        ...
        <article className="surface-card category-card category-card--fixed">
          ...
          <p className="category-card__note">Ideal para ninera, alquiler, internet, colegio o cualquier pago que no tiene fecha de cierre.</p>
        </article>
      </section>
